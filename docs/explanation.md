...
```
It's worth noting that *class names start with a valid CSS property name*. The downside of the decision is class names too long! Nobody likes type too long names. And to solve this issue we introduce **four simple rules for abbreviating ALL property names**. Let's skip the details and see the final result:
```CSS
.c-white {color: white;}

.bg-clouds {background: url('clouds.jpg');}
```
Ok, looks better. Take into account two things: (a) despite of `c` and `bg` are *shortcuts* they're considered as a *valid* property names in context of element; (b) each element should be written in *one line* to take up less space. Looking ahead having standardized and short prefixes is important for the several reasons:

>
- we can see what element does by its class name
- we can read others code and always grasp the general idea of layout
- we can use useful IDE's autocomplete function
- using short names and a lot of elements make resulting HTML code less verbose

It's a good practice to add spaces between curly brackets `{ property: value; }`, in SEM practice there is no strict rules. However I prefer not to do so, because manage them takes time and give no significant advantages. If to be honest, the dense view of a lot of elements seems more pleasant to me.
...
```
Nothing special here. In total we have *4 sets*, *6 elements* and *2 mixes*.
...
Now you have an idea how SEM looks like. At this step you can ask: and so what? We need to create dozens of classes with one property and somehow to create complex layout?! (Are you crazy?!) Looking ahead the answer is yes (for both:). Now you can add: but what about mixes?  What is special with them and if so why we need them at all? How do I know when to create mix or element? ... But let's not get ahead of ourselves and focus on elements for a while. I will explain how to organize them and why it's worth coping with dozens of them.
...
```
It may looks scary, but don't hurry with the conclusions. You can notice that elements are well-organized:

>
- at the beginning sets with box-model properties, then visual related ones;
- margin & padding sets become strict collections of graduated values (small, medium large etc);
- background set has pseudo-class elements with an appropriate names goes after double dash `*--hover`;

In this way, I want to emphasize that creating elements isn't *chaotic and messy* process. In contrast, creating elements takes some patience, premeditate and conventions. But I can state if you master this technique (and CSS in general as you can guess) you will literally draw your GUI like in WYSIWYG editors. But for now I intentionally skip some details, because we need to go further.

So, if you look more carefully at the previous example you may notice we can already "draw" some GUI-elements. For example: blockquote, 2 buttons (blue, green), successful/warning/error messages, inputs etc. Let's create some of them:
```HTML
...
<body class="c-black">
  <!-- blockquote -->
...
[Demo](http://jsfiddle.net/snfvk69t/1/). Ok, looks pretty good :) This is how SEM *may* look like in real apps. It may seems like the code are messy and verbose, but it are matter of habit and HTML organization (which we optimize using Web Components a bit later). From now on I want to state some interesting assumptions:

>
- The more elements we create, the less elements we will need in the future;
- The more elements we have, the more unique GUI-elements we can "draw" (with any complexity and without creating new classes);
- The more strictly we organize sets of elements, the more opportunities we have to reuse and share the code (or read others code) in the future.
